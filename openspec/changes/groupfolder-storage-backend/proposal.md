...
- Add an admin setting `launchpad.content_storage` with values `db` (default) or `groupfolder`.
- Auto-create a GroupFolder named "LaunchPad" on first GroupFolder-backed write or via explicit admin trigger, with ACL rules restricting access to intended dashboard members.
- Persist each dashboard's content (widget tree, layout, metadata) as a single JSON file at `<LaunchPad>/<locale-or-empty>/<dashboard-uuid>.json`.
- Read/write via Nextcloud's `IRootFolder` API — never direct filesystem paths.
- Provide a one-time migration command `launchpad:storage:migrate-to-groupfolder` that copies all existing dashboards from the DB to the GroupFolder, idempotent and safe to re-run.
...
- `admin-settings`: adds one new setting key `launchpad.content_storage` (enum: `db` or `groupfolder`, default `db`).
...
- `lib/Command/ToggleStorageSetting.php` — optional admin helper to change `launchpad.content_storage` setting with validation
...
- `launchpad:storage:migrate-to-groupfolder` — idempotent bulk migration from DB to GroupFolder
- `launchpad:storage:toggle-backend {db|groupfolder}` — change the admin setting (optional, for convenience)
...
- Zero-impact by default — all existing dashboards remain in the DB (`launchpad.content_storage = 'db'` is default).
...
- Rollback: if an operator switches `launchpad.content_storage` back to `db`, the DB records are still present (migration does not delete them).
