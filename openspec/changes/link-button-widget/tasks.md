...
- [~] Task 11: Playwright — createFile flow end-to-end (modal opens → POST → opens Files tab); external link opens in `_blank` tab — **DEFERRED**. The three click branches (external, internal-function, createFile), modal-open + POST + placeholder swap, validation, and the disabled-while-in-flight behaviour are covered by the Vitest suite (Task 10). The remaining piece is browser tab/window behaviour (`target="_blank"` + the Files app deep-link), which Playwright is the right tool for; tracked under the gate-19 honest-coverage program so the link-button Playwright lands alongside the other widget specs. `tests/e2e/link-button-widget.spec.ts` will: render a link-button with `actionType: external` → click → assert new tab; render a link-button with `actionType: createFile` → click → assert modal opens, POST to `/api/files/create`, then Files tab opens with the new file selected
