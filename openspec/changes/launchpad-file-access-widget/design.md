# Design — File Access Widget for LaunchPad Dashboard

## Context

LaunchPad users work with dossiers (cases) that have associated files (documents, evidence, permits). Today, accessing these files requires:
...
- Query a custom `launchpad_dossier_files` junction table. Rejected — ADR-022 requires consuming OR's abstractions; custom tables duplicate OR's relation logic.
...
- Embed the widget in a fixed section above the grid. Rejected — breaks the drag-drop layout paradigm LaunchPad users expect.
- Load files on every dashboard render. Rejected — unnecessary API calls; fetch once per dossier + cache.

**Rationale**: GridStack integration keeps the widget consistent with other LaunchPad widgets (drag-to-reposition, resizable, add/remove via menu). Dossier-change detection reloads data when context shifts.
...
1. **What is the "current dossier" context on the dashboard?** Does the widget require the user to select a dossier first, or does LaunchPad have a default dossier per user? (Answer: design assumes LaunchPad provides a `selectedDossierId` from the dashboard state or route params.)
