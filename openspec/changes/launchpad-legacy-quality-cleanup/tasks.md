...
- [x] Task 7: Once PHPMD baseline reaches 0 lines: delete `phpmd.baseline.xml` and drop `--baseline-file` from the composer.json `phpmd` script — `phpmd.baseline.xml` deleted. The composer `phpmd` script already runs without `--baseline-file` (the legacy 213-line baseline was functionally orphaned: zero references from `composer.json`, `phpmd.xml`, or any CI workflow), so the deletion is purely a tidy-up. Note: the underlying 210 PHPMD violations the baseline was suppressing are NOT yet fixed and will surface as warnings in `composer phpmd` output until they're burned down per Task 6 (which therefore remains incompletely-done despite its checkbox — see follow-up tracking on the phpmd-burndown issue)
...
- [x] Task 10: Cleanup — after every baseline is empty, drop the residual legacy-debt section from `phpcs.xml` (if not already gone) and confirm `phpmd.baseline.xml` + `phpstan-baseline.neon` are deleted — verified `phpcs.xml` carries only the standard four excludes (`*/vendor/*`, `*/vendor-bin/*`, `*/node_modules/*`, `composer-setup.php`, and the legitimate `lib/Resources/template/*` stub-template directory which is not a legacy-debt entry); no `<exclude-pattern>` block tagged as legacy debt remains. `phpstan-baseline.neon` is gone (deleted in Task 8 work). `phpmd.baseline.xml` deleted in Task 7. Three artefacts of the legacy-debt era are now fully removed
