...
|------------|-----------|-----|
| App-manifest schema | `@conduction/nextcloud-vue` `src/schemas/app-manifest.schema.json` | Single source of truth per `hydra/openspec/changes/adopt-app-manifest/`. LaunchPad MUST NOT fork or extend the schema in-tree — extensions go upstream. |
...
| Tenant context | `useTenantContext()` from nc-vue (pending `multi-tenancy-context` change) | When it ships, LaunchPad widgets reuse it instead of rolling org-scope logic. |
...
| Permission model on dashboards | LaunchPad own `oc_launchpad_dashboards.permissions` column | Local concept (view_only / add_only / full) predates OR's per-object RBAC. Keep local; optional runtime delegation. |

### What we deliberately do NOT reuse

- **OR's `RegisterResolverService`** — LaunchPad has no PHP-side OR
  ...
  controllers) and pipelinq (8 services) — LaunchPad is not on that list.
- **OR's lifecycle annotations** — LaunchPad dashboards are not
...
- **OR's `Translation` table** — LaunchPad translations remain in
  Nextcloud's `IL10N` system (`l10n/{nl,en}.js`). LaunchPad does not
  store translatable user-authored content; widget labels are
  developer-authored.
- **Hydra's per-app `adr-000`** — per ADR-022, apps MUST NOT carry an
  ADR that re-asserts cross-app conventions. LaunchPad already does not
  ...
  "pages": [
    ...
    },
    {
      ...
      "config": {
        "register": null,
        "schema": null,
        "source": "launchpad:oc_launchpad_admin_settings",
...
Notes:
- `config.source: "launchpad:oc_launchpad_admin_settings"` is a LaunchPad-local
  source URN. The renderer treats it opaquely; only the
  `AdminTemplatesIndexAdapter` LaunchPad registers via `customComponents`
  understands it.
- `config.{widgets, layout}: "@runtime"` is a sentinel meaning "fetch
  from LaunchPad backend per-dashboard". The dashboard page-type
...
useAppManifest('launchpad', bundledManifest)
```
...
- **Permission levels** (`view_only` / `add_only` / `full`) live on
  `oc_launchpad_dashboards.permissions`.
- **Runtime OR delegation (OPTIONAL)** — when a dashboard's
  `permissions.delegate` field references an OR-backed object, LaunchPad
...
- Templates persist in `oc_launchpad_admin_settings`.
...
| Operator runs LaunchPad without OR (intended path) | `useOrFeatureDetect()` returns `enabled.value === false`; widgets render empty state. Smoke tested in Phase 8.2. |
| Operator runs LaunchPad with OR but tenant context absent | `useTenantContext()` (when shipped) exposes `null`; widgets render OR data without tenant filter (server still enforces per-session). |
...
| Constants rename in Phase 5.2 breaks BC | Old constants kept as aliases; no consumers external to LaunchPad. |
...
2. **Q2 — Backend `/api/manifest` endpoint.** Per
   ...
   admin-customised overrides (menu order, hidden pages). LaunchPad has
...
5. **Q5 — `runtime-or-consumption` spec home.** This change creates a
   new local capability under `launchpad/openspec/specs/`. Should it
...
6. **Q6 — `ColumnTypeRegistry` future.** Stream 4 flagged the
   constants as "hardcoded". Is there a future where LaunchPad column
...
7. **Q7 — manifest version cadence.** ADR-024 specifies semver-on-
   content for `manifest.version`. Should LaunchPad bump on every menu
