...
const shareUrl = (uuid: string) => `/index.php/apps/launchpad/api/dashboards/${uuid}/public-share`
const sharesUrl = (uuid: string) => `/index.php/apps/launchpad/api/dashboards/${uuid}/public-shares`
 ...
 */
async function createDashboard(api: APIRequestContext, label: string): Promise<string> {
	const res = await api.post(DASHBOARDS, { data: { name: `E2E ${label} ${Date.now()}` } })
...
): Promise<{ id: number, token: string, passwordRequired: boolean, expiresAt: string | null }> {
...
test.describe('public share creation options', () => {
	test('a share created with no options has no password and no expiry, and serves anonymously', async ({ baseURL }) => {
		...
		expect(share.passwordRequired, 'a share created with `{}` must not be password-gated').toBe(false)
		expect(share.expiresAt, 'a share created with `{}` must never expire').toBeNull()
	...
	test('a share created with a password is gated, and the password is never echoed back', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		const anon = await anonymousApi(baseURL!)
		...
		expect(share.passwordRequired, 'a share created WITH a password must report passwordRequired').toBe(true)
		...
		expect(raw, 'passwordHash must not be serialised at all').not.toContain('passwordHash')
		...
		expect(gated.status(), 'a password-gated share must not render without one').toBe(401)

		await admin.dispose()
		await anon.dispose()
	})

	test('a share created with an expiry carries it back, and expiry decides validity in both directions', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		const anon = await anonymousApi(baseURL!)
		...
		expect(expired.expiresAt, 'a supplied expiry must be stored, not dropped').not.toBeNull()
		expect(valid.expiresAt, 'a supplied expiry must be stored, not dropped').not.toBeNull()
		expect(never.expiresAt, 'a share created without an expiry must have none').toBeNull()
		...
		expect(past404.status(), 'a share whose expiry has elapsed must 404').toBe(404)
		...
		expect(neverOk.status(), 'a share with expiresAt NULL must not have expiry logic applied to it').toBe(200)

		await admin.dispose()
		await anon.dispose()
	})

	test('two shares on one dashboard are independent — revoking one leaves the other live', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		const anon = await anonymousApi(baseURL!)
		...
		expect(second.token, 'two shares must not share a token').not.toBe(first.token)
		expect(first.passwordRequired).toBe(false)
		expect(second.passwordRequired, 'per-share password settings must be independent').toBe(true)
...
test.describe('owner-only management endpoints', () => {
	test('the list returns exactly the live shares, by token, and drops the revoked one', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		...
		expect(await empty.json(), 'a dashboard with no shares must list none').toEqual([])

		const live = [await createShare(admin, uuid), await createShare(admin, uuid), await createShare(admin, uuid)]
		...
		expect(tokens.slice().sort(), 'the list must be exactly the three live shares').toEqual(
			live.map(s => s.token).slice().sort(),
		)
		expect(tokens, 'a soft-revoked share must not appear in the list').not.toContain(doomed.token)
		...
		for (const entry of listed) {
			for (const field of ['id', 'token', 'url', 'passwordRequired', 'expiresAt', 'viewCount', 'lastViewedAt']) {
				expect(entry, `every listed share must carry '${field}'`).toHaveProperty(field)
			}
			expect(entry.token, 'the token must be returned in full, not redacted').toEqual(expect.any(String))
		}

		await admin.dispose()
	})

	test('revoking is idempotent — the second DELETE is still 204', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
	...
	test('a non-owner cannot create, list or revoke shares the owner can', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		...
		const tokens = (await after.json() as Array<{ token: string }>).map(s => s.token)
		expect(tokens, 'a refused create must not have created anything').toEqual([share.token])
...
test.describe('the password gate', () => {
	test('the wrong password is refused, the right one unlocks, and either transport carries it', async ({ baseURL }) => {
		const admin = await apiAs(baseURL!, ADMIN)
		const anon = await anonymousApi(baseURL!)
		...
		expect(gatedBody.passwordRequired, 'the refusal must say what is missing').toBe(true)
		expect(gatedBody, 'a refused render must not carry the dashboard').not.toHaveProperty('dashboard')
		expect(gatedBody, 'a refused render must not carry its placements').not.toHaveProperty('placements')
		...
		expect((await wrong.json()).access, 'a wrong password must not grant access').toBe(false)
		...
		expect((await right.json()).access, 'the correct password must grant access').toBe(true)
		...
		const viaQuery = await anon.get(`${dataUrl(share.token)}?password=${encodeURIComponent(password)}`)
		expect(viaQuery.status(), await viaQuery.text()).toBe(200)
		expect(await viaQuery.json(), 'an unlocked render must carry the dashboard').toHaveProperty('dashboard')

		const viaHeader = await anon.get(dataUrl(share.token), {
			headers: { 'X-Share-Password': password },
		})
		expect(viaHeader.status(), await viaHeader.text()).toBe(200)
		expect(await viaHeader.json(), 'an unlocked render must carry the dashboard').toHaveProperty('dashboard')
