...
- **expired_locks**: rows in `oc_launchpad_dashboard_locks` where `expiresAt < now()`
- **expired_share_tokens**: rows in `oc_launchpad_public_shares` where `expiresAt < now()` OR `revokedAt < (now() - 30 days)`
- **orphaned_widget_assets**: files in `LaunchPad/Imports/*` and `LaunchPad/icons/*` (Nextcloud file storage) not referenced by any current dashboard or widget config
- **orphaned_metadata_values**: rows in `oc_launchpad_metadata_values` where `fieldId` does NOT exist in `oc_launchpad_metadata_fields`
- **orphaned_widget_placements**: rows in `oc_launchpad_widget_placements` where `dashboardId` does NOT exist in `oc_launchpad_dashboards`
- **orphaned_feed_tokens**: rows in `oc_launchpad_feed_tokens` where `userId` no longer exists in `oc_users`
- **orphaned_role_assignments**: rows in `oc_launchpad_role_assignments` where `userId` (or `groupId`) no longer exists in `oc_users` (or `oc_groups`)
- **dangling_dashboard_translations**: rows in `oc_launchpad_dashboard_translations` where `dashboardUuid` no longer exists in `oc_launchpad_dashboards`
...
- GIVEN a LaunchPad installation with: 3 expired locks, 2 expired share tokens, 5 orphaned widget assets, 1 orphaned metadata value
...
- GIVEN a LaunchPad installation with 3 expired locks and 2 expired share tokens
...
- WHEN an administrator runs `php occ launchpad:cleanup:scan --dry-run`
- THEN the system MUST:
...
- GIVEN a LaunchPad installation with 3 expired locks
...
- GIVEN LaunchPad is configured with auto-purge categories = ["expired_locks", "expired_share_tokens"]
...
- THEN the system MUST:
  ...
  - Log the result in the LaunchPad activity log

#### Scenario: Admin changes auto-purge category list
- GIVEN the admin navigates to Settings > Administration > LaunchPad > Cleanup
- WHEN they uncheck "Auto-purge expired locks" and click Save
- THEN the admin setting `launchpad.cleanup_auto_purge_categories` MUST be updated
...
- WHEN the admin views Settings > Administration > LaunchPad > Cleanup
...
- THEN the system MUST emit one NC activity event with:
  - Event type: `launchpad_cleanup_purge`
