...
const MEMB = {
	user: process.env.NC_MEMBER_USER ?? 'member',
	pass: process.env.NC_MEMBER_PASS ?? 'member',
}
const NOMEM = {
	user: process.env.NC_NONMEMBER_USER ?? 'nonmember',
	pass: process.env.NC_NONMEMBER_PASS ?? 'nonmember',
}
...
test.describe('Group-shared dashboards (REQ-DASH-011..014)', () => {
	...
	test('T1: admin creates group-shared dashboard and member sees it in /visible', async () => {
		...
		const created = (createBody as { data?: { dashboard?: { uuid?: string } } })
			.data?.dashboard
		...
		const visibleRes = await memberCtx.get(
			`${BASE}/index.php/apps/launchpad/api/dashboards/visible`,
		)
		expect(visibleRes.status(), 'GET /visible should be 200 for member').toBe(
			200,
		)

		const visibleBody = (await visibleRes.json()) as {
			data?: Array<{ uuid: string; source: string }>
		}
		...
		expect(
			found,
			`Dashboard ${createdUuid} must appear in member's /visible`,
		).toBeTruthy()
		expect(found?.source, 'Source must be "group" for group-matched rows').toBe(
			'group',
		)
	...
	test('T2: user with zero group memberships sees default-group dashboards in /visible', async () => {
		...
		const visibleRes = await nomemCtx.get(
			`${BASE}/index.php/apps/launchpad/api/dashboards/visible`,
		)
		expect(visibleRes.status(), 'GET /visible should be 200 for nonmember').toBe(
			200,
		)
		...
		expect(
			found,
			`Default-group dashboard ${defaultUuid} must appear in nonmember's /visible`,
		).toBeTruthy()
		expect(
			found?.source,
			'Source must be "default" for default-group rows',
		).toBe('default')

		// Clean up.
		await adminCtx.delete(
			`${BASE}/index.php/apps/launchpad/api/dashboards/group/default/${defaultUuid}`,
		)
		await adminCtx.dispose()
	...
	test('T4: admin rename propagates to member on next /visible call', async () => {
		...
		expect(
			found?.name,
			'Renamed dashboard must have the new name in /visible',
		).toBe(NEW_NAME)
