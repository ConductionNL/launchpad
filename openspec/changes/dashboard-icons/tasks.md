...
- [ ] Task 8: Add a docblock on `lib/Db/Dashboard.php`'s `icon` field describing the convention (NULL or registry name or URL); confirm no migration is needed (column already exists on `oc_launchpad_dashboards`)
