...
- [x] 4.2 Add `Version001006Date20260502000000.php` to add the `icon` column on `launchpad_dashboards` (the proposal's "no migration needed" was incorrect — the column did not previously exist; confirmed via `DashboardTableBuilder.php`). Also added the column to `DashboardTableBuilder` for fresh installs.
...
- [ ] 5.4 Visual snapshot (Storybook or equivalent): launchpad has no Storybook infrastructure, deferred — Vitest covers behaviour and the bundle audit covers tree-shake; visual coverage is best added in a follow-up Storybook spec.
