...
- [ ] 1.1 Create `lib/Activity/Extension.php` implementing `OCP\Activity\IProvider`; declare class constant `APP_ID = 'launchpad'`
...
- [ ] 1.3 Implement `getIcon(string $eventType): string` — return full URL to the per-type SVG from `img/activity/`; fall back to the generic LaunchPad icon for unknown types
- [ ] 1.4 Register `\OCA\LaunchPad\Activity\Extension` in `appinfo/info.xml` under `<activity>`
...
- [ ] 3.2 Inside `publish()`: build `IEvent` via `IManager::generateEvent()`; set `app`, `type`, `author`, `object` (`object_type = 'launchpad_dashboard'`, `object_id = $dashboardUuid`), `subject`, `message`, `link`, and `timestamp`; call `IManager::publishActivity($event)` — never bypass `IManager`
...
- [ ] 7.2 Gate global fan-out through `DebounceHelper::allowGlobalFanout(string $dashboardUuid, string $eventType): bool` — returns true at most once per 15 minutes per `(dashboardUuid, eventType)` combination; uses APCu key `launchpad_act_global_{dashboardUuid}_{eventType}`
...
- [ ] 8.1 Create `lib/Activity/DebounceHelper.php` with `allowReaction(string $actorUserId, string $dashboardUuid): bool` — returns true at most once per 15 minutes per `(actorUserId, dashboardUuid)` pair; APCu key `launchpad_act_react_{actorUserId}_{dashboardUuid}`; TTL 900 seconds
...
- [ ] 9.5 Message templates (optional detail lines) use the same key pattern `launchpad_act_msg_{eventType}` — empty string is acceptable for non-comment events; `dashboard_commented` message MUST include the first 200 characters of the comment body
...
- [ ] 10.2 `Extension::getIcon(string $eventType)` MUST return an absolute URL via `\OCP\IURLGenerator::imagePath('launchpad', "activity/{$eventType}.svg")` — no hardcoded strings
- [ ] 10.3 Provide a generic fallback `img/activity/launchpad.svg` returned when `$eventType` is not one of the 13 known constants
