---
capability: divider-widget
delta: true
status: draft
---

# Divider Widget — Delta from change `divider-widget`
...
- **AND** the widget MUST have a display name (e.g., "Divider") and an icon
...
- **AND** the widget provider MUST return widget metadata: id, display name, icon URL, and the `supportsV2` flag set to `true`
...
- **AND** mydash_divider MUST not be marked as requiring special permissions
...
- **AND** the form MUST provide clear visual labels for each field
...
#### Scenario: Dashed and dotted line styles
...
- **AND** dotted lines MUST also be supported (CSS `border-style: dotted`)
...
- **GIVEN** the user is adding a new divider widget to a dashboard
- **WHEN** the "Add Widget" modal opens and the user selects the divider widget
- **THEN** the modal MUST pre-populate `gridWidth = max dashboard width` (typically the full dashboard column span)
