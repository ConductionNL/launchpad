...
Dashboards are shared workspaces within teams and organizations. Currently LaunchPad offers no way for users to discuss or annotate a dashboard — there is no thread for "why is this widget red?" or "we should change this metric next sprint". This change introduces threaded, one-level-deep comments on dashboards, leveraging Nextcloud's native `ICommentsManager` infrastructure (already used for file comments, talk integration, etc.) so that Nextcloud administrators get unified comment storage, notifications, and audit trails across the platform. Comments are persisted via Nextcloud's existing comment storage layer with no custom database table required.

## What Changes

- Use `ICommentsManager` with object type `launchpad_dashboard` and object id = dashboard UUID to store all comments.
- Add a nullable `commentsEnabled SMALLINT(0/1) NULL` column on `oc_launchpad_dashboards` to support per-dashboard comment toggle (NULL = follow global setting, 1 = force-on, 0 = force-off).
- Add global admin setting `launchpad.comments_enabled_default` (defaults to true) that applies to all dashboards where `commentsEnabled IS NULL`.
...
- `lib/Migration/VersionXXXXDate2026...AddCommentsEnabledColumn.php` — add `commentsEnabled` column to `oc_launchpad_dashboards`
