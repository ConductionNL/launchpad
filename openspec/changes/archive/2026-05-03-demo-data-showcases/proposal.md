...
- **NEW** Admin UI: "Demo data" tab in LaunchPad admin section listing showcases with preview thumbnail, description, and per-showcase "Install" / "Uninstall" button.
...
- **NEW** CLI commands: `php occ launchpad:demo-showcases:install <id> [--force]` and `php occ launchpad:demo-showcases:list`. The `--force` flag bypasses the idempotency guard for reinstallation.
...
- Database: NO new tables — showcases are read-only files, installations stored in existing `oc_launchpad_dashboards` table (with type `group_shared`, groupId `default`).
