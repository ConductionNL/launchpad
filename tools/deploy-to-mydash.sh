...
if [ "$JS_ONLY" -eq 0 ]; then
	...
	# The <id> MUST equal the install dir name (mydash), else NC rejects the app
	# with "appinfo file cannot be read" (id/dir mismatch) and app:enable fails —
	# leaving the dev env without the dashboard.
	sed -i 's#<id>launchpad</id>#<id>mydash</id>#' "$STAGE/appinfo/info.xml"
