...
- Keep the widget type self-contained — no new database tables, no new backend services, no new API endpoints. The persisted content lives in the existing `oc_launchpad_widget_placements.styleConfig` JSON column.
...
**Decision**: The widget's content lives at `oc_launchpad_widget_placements.styleConfig` as `{type: 'text', content: {...}}`. No schema migration.

**Alternatives considered:**

- Add a dedicated `oc_launchpad_text_widgets` table keyed by placement id. Rejected because it (a) requires a new mapper, service, and JOIN on every placement read, and (b) hardcodes one widget type into the schema — inconsistent with how other potential built-in widget types (e.g. spacer, separator, image) would land later.
...
**Decision**: When `text.trim() === ''`, render an italic `t('launchpad', 'No text content')` in `var(--color-text-maxcontrast)`. The wrapper still occupies 100% of the cell.
...
**Rationale**: Italic + low-contrast colour reads instantly as "empty state", matches the convention used elsewhere in LaunchPad (empty-tile placeholder), and keeps the cell as a valid GridStack drop target.
