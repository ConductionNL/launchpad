...
class AdminSettingKeyTest extends TestCase
{
    ...
    public function testAllCasesHaveNonEmptyStringValue(): void
    {
        ...
        $this->assertNotEmpty($cases, 'AdminSettingKey must have at least one case.');

        foreach ($cases as $case) {
            $this->assertIsString($case->value);
            $this->assertNotSame('', $case->value, "Enum case {$case->name} has an empty value.");
        }
    }
    ...
    public function testEnumValuesAreUnique(): void
    {
        ...
        $this->assertSame(count($values), count(array_unique($values)), 'AdminSettingKey enum values are not unique.');
    }
    ...
    {
        $this->assertSame($enumCase->value, $constant, "AdminSetting::{$enumCase->name} alias does not match enum value.");
    }
    ...
    public function testFromWorksForEveryAlias(): void
    {
        ...
        foreach ($aliases as $alias) {
            ...
            $this->assertInstanceOf(AdminSettingKey::class, $case);
            $this->assertSame($alias, $case->value);
        }
    }
}//end class
