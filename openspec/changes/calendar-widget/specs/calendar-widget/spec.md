# Spec: calendar-widget

**Status:** proposed
**Scope:** mydash
**Tier:** widget-capabilities
**Depends on:** widgets, widget-add-edit-modal, runtime-shell, grid-layout, permissions; external services: OCP\Calendar\IManager, Nextcloud ICS feeds, sabre/vobject RRULE expansion

## Purpose

Surface Nextcloud Calendar events aggregated from internal calendars and external ICS feeds on a MyDash dashboard tile. The widget supports four view modes (month, week, agenda, upcoming-list), server-side ICS caching with admin-configurable TTL, HTTPS-only external feed fetching with optional hostname allow-list, per-calendar color coding, and graceful degradation when external sources fail.

The widget is read-only; event creation/editing redirects to the full Calendar app.

## ADDED Requirements

### REQ-CAL-001: Widget registration
...
### REQ-CAL-002: Placement configuration
...
- **AND** each entry MUST be a valid HTTPS URL ending in `.ics` or a feed-like path
...
### REQ-CAL-004: Recurring event expansion
...
### REQ-CAL-005: External ICS caching
...
### REQ-CAL-006: External ICS allow-list
...
### REQ-CAL-007: View-time access control
...
### REQ-CAL-010: Empty state and feedback
