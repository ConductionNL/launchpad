# LaunchPad — adopt OR abstractions (manifest, runtime-only OR consumption)

## Why

The 2026-05-03 OR-abstraction audit (`.claude/audit-2026-05-03/`) flagged
adoption gaps across all eight Specter-pipeline apps. For LaunchPad the gaps
fall into three categories:

1. **Manifest adoption** — LaunchPad currently has no architectural
   `src/manifest.json` and wires its router by hand. Per the
   fleet-wide `adopt-app-manifest` change in `hydra/openspec/changes/`
   and the migration order in **ADR-024**, LaunchPad is the recommended
...
2. **Spec staleness** — `dashboard-sharing/spec.md` and
   ...
   per-object RBAC vs LaunchPad's own `oc_launchpad_*` tables) owns each
...
LaunchPad is a BI / dashboarding tool. Per
`feedback_launchpad-no-or-dependency.md`, LaunchPad MUST NOT declare a hard
...
Any OR data LaunchPad displays is consumed exclusively over the **runtime
GraphQL / REST API** with graceful degradation when OR is absent.

This makes LaunchPad structurally different from the other adoption
...
- Add `src/manifest.json` describing the current LaunchPad UI: top-level
...
- Set `dependencies: []` (explicit empty — no install-time OR/OC
  dependency).
- Wire `useAppManifest('launchpad', bundled)` in `src/main.js`. Start at
...
- Rewrite `openspec/specs/dashboard-sharing/spec.md` to describe LaunchPad
  permission levels (`view_only` / `add_only` / `full`) as native
  LaunchPad concepts (`oc_launchpad_dashboards.permissions`), with an
...
- Rewrite `openspec/specs/admin-templates/spec.md` to declare that
  admin templates are stored in LaunchPad's own `oc_launchpad_admin_settings`
  table (NOT in OR) and document the rationale: LaunchPad must work
...
- `ColumnTypeRegistry.php:31-45` — keep `TYPE_INTEGER`/`TYPE_BOOLEAN`/
  `TYPE_STRING` constants local. They legitimately model LaunchPad column
...
boot. LaunchPad cannot. As a dashboarding tool it must:
...
- Feature-detect OR at runtime and unlock OR-backed widgets only when
  available.
- Avoid stamping OR identifiers (register IDs, schema IDs) in LaunchPad's
  own database — LaunchPad dashboards that reference OR data store
  references *symbolically* (e.g. `{"source": "openregister:register/schema"}`)
  and resolve at render time.

The current adoption gap means LaunchPad dashboards that surface OR data
...
policy keeps LaunchPad deployable in OR-less Nextcloud installs.

## Proposed Solution

A single `launchpad-adopt-or-abstractions` change that:
...
2. Documents (in a new shared `runtime-or-consumption` spec capability)
   the pattern for *runtime-only* OR consumption that LaunchPad uses, so
...
No code outside LaunchPad changes. No PRs are opened by this change — it
...
- Building OR-backed dashboard widgets in this change. Widget work
  ...
  change documents.
- Migrating LaunchPad's own data into OR. LaunchPad owns `oc_launchpad_*`
  tables; that ownership stays.
- Tier 4 manifest adoption (full `CnAppRoot` shell). Tier 1 first;
  Tier 3 in follow-up; Tier 4 only if/when LaunchPad's bespoke admin
...
- `feedback_launchpad-no-or-dependency.md` — the policy this change
  encodes.
