...
- Cache external ICS fetches for 30 minutes using Nextcloud's `ICache`, tunable via admin setting `launchpad.calendar_widget_ics_cache_ttl_seconds`.
- Enforce an allow-list of external ICS hosts via admin setting `launchpad.calendar_widget_allowed_ics_hosts` (JSON array of hostnames; empty = all allowed; populated = restricted to those hosts). URLs not matching the allow-list are silently skipped with a warning logged.
...
- `calendar-widget` — A new LaunchPad dashboard widget capability providing merged calendar views from Nextcloud and external ICS sources.
...
- `lib/Migration/VersionXXXXDate2026...AddCalendarWidgetSettings.php` — schema migration adding app config settings (`launchpad.calendar_widget_*` keys).
