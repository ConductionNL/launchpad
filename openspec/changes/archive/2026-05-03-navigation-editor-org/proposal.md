...
- `lib/AdminSettingsService.php` — extend to store/retrieve `launchpad.org_navigation_position` global setting (scalar; unchanged)
- `lib/OrgNavigationFileService.php` — new service: read/write `appdata/launchpad/org-navigation-{lang}.json` (up to 5 MB), validate tree depth (max 3), URL sanitise, filter tree by user's group memberships via `IGroupManager`, resolve icon URLs
...
- **Storage** — per-language JSON files at `appdata/launchpad/org-navigation-{lang}.json` (up to 5 MB each; not stored in app-config). v1 supports `nl` and `en`. The tree is wholesale-replaced per language via `PUT /api/admin/org-navigation?lang={lang}`. Each node is a UUID-identified object with `label`, `icon`, `url` (optional for section nodes), `openInNewTab`, `groupVisibility` (null = all users; array = restrict to groups — a LaunchPad-specific field, not inherited from the reference implementation), and `children`.
...
- **Rendering** — Vue 3 SFC `OrgNavigationPanel.vue` mounted by `App.vue`. Responsive: rail at viewport ≥800px, hamburger+drawer at <800px. Position is configurable via `launchpad.org_navigation_position` (left, right, top, hidden; default hidden). Active item detection based on URL match.
...
- `admin-settings` — gain new global settings key: `launchpad.org_navigation_position` (scalar; the tree itself is stored as filesystem files, not in app-config)
