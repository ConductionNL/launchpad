...
LaunchPad inherits its ADRs from the ConductionNL platform-wide set. There
are no LaunchPad-specific ADRs today — every rule that applies to LaunchPad
...
When Hydra's builder + reviewer containers operate on a LaunchPad PR, they
...
LaunchPad's current compliance posture against the 23 ADRs is tracked in
