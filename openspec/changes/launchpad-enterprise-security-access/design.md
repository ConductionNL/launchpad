...
LaunchPad currently enforces access control at the user level: an authenticated user is either an admin (sees all dashboards) or a guest (sees no dashboards). This binary model makes it impossible to grant selective dashboard access to external collaborators without making them Nextcloud administrators. Enterprises deploying LaunchPad for multi-stakeholder governance (city councils, university boards, project consortia) need role-based visibility so that board members, secretaries, and external observers can each see only the dashboards and widgets relevant to their role, without admin privileges.
...
**`GET /index.php/apps/launchpad/api/roles`**
...
**`DELETE /index.php/apps/launchpad/api/impersonate`**
...
**Trade-off:** Admins must manage roles in LaunchPad settings, not in Nextcloud group admin UI. Acceptable because roles are dashboard-specific.
...
9. **Seed data** — `lib/Settings/launchpad_register.json` includes 3 default roles (chair, secretary, member) and example assignments

## Seed Data

**In `lib/Settings/launchpad_register.json`:**
