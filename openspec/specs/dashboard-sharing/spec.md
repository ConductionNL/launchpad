...
### REQ-SHARE-014: Optional orphan-share cleanup migration (opt-in)

The app MUST ship a `SimpleMigrationStep` (`Version001006Date20260430130000`) that, when an admin has opted in via the `cleanup_orphan_shares` app config flag, removes share rows whose `share_with` no longer resolves to a real Nextcloud user (for `share_type='user'`) or group (for `share_type='group'`). The migration MUST:

- Default to **no-op** when the flag is absent or set to `false`, emitting only an `IOutput::info` message explaining how to opt in.
- Read the gate via `IConfig::getAppValue('mydash', 'cleanup_orphan_shares', 'false')`.
- Resolve users via `IUserManager::get($uid)` and groups via `IGroupManager::get($gid)`. Anything returning `null` is considered orphaned.
- Run inside `postSchemaChange` (no schema change is required).
- Emit an `IOutput::info` line at the end stating how many user-shares and group-shares were deleted.

The migration is intentionally opt-in to avoid surprise data deletion in federated, multi-realm, or shadow-account environments where `IUserManager::get($uid)` may transiently return null for a uid that is still valid.

#### Scenario: Default behaviour is a no-op

- GIVEN no value for the `mydash`/`cleanup_orphan_shares` app config
- AND there are share rows referencing non-existent users
- WHEN the migration is executed via `occ migrations:execute mydash 001006Date20260430130000`
- THEN the migration MUST report "skipping orphan-share cleanup"
- AND no rows MUST be deleted

#### Scenario: Opt-in deletes orphan rows

- GIVEN `occ config:app:set mydash cleanup_orphan_shares --value=true`
- AND there is one share row with `share_type='user', share_with='ghost-user-x'` where `ghost-user-x` does not resolve via `IUserManager`
- AND there is one share row with `share_type='group', share_with='ghost-group-x'` where `ghost-group-x` does not resolve via `IGroupManager`
- WHEN the migration is executed
- THEN both rows MUST be deleted
- AND the migration MUST emit an info line of the form `mydash: orphan share cleanup deleted 1 user-share rows and 1 group-share rows`
