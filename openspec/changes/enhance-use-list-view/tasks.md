...
- [ ] `npm run build` succeeds in all three projects
- [ ] Manual test: list view loads, search works, sidebar filters update results, sorting works, pagination works
- [ ] Manual test: creating a new object via the form and saving redirects correctly (useDetailView)
- [ ] Manual test: deleting an object navigates back to the list (useDetailView)
- [ ] No `setupSidebar` / `teardownSidebar` / manual `fetchCollection` remains in migrated files
