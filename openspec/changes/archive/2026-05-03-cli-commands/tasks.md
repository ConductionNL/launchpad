...
- [ ] 1.3 Audit log helper MUST write `[launchpad] cli <command> <args> exitCode=<n> durationMs=<ms> byUser=<uid|cli>` to NC main log via `\OC::$server->getLogger()` regardless of success or failure
- [ ] 1.4 Add SPDX headers inside the file docblock on all new PHP files (gate-spdx compliance)

## 2. Dashboard management commands

- [ ] 2.1 Create `lib/Command/DashboardListCommand.php` implementing `launchpad:dashboard:list`
...
- [ ] 2.2 Create `lib/Command/DashboardShowCommand.php` implementing `launchpad:dashboard:show <uuid>`
  - Validates UUID format (exit 2 on malformed)
  - Returns exit 4 if dashboard not found
  - Outputs full widget tree + metadata as JSON
- [ ] 2.3 Create `lib/Command/DashboardDeleteCommand.php` implementing `launchpad:dashboard:delete <uuid> [--cascade]`
...
- [ ] 2.4 Create `lib/Command/DashboardDebugShareCommand.php` implementing `launchpad:dashboard:debug-share <uuid>`
...
- [ ] 3.1 Create `lib/Command/UserRevokeFeedTokenCommand.php` implementing `launchpad:user:revoke-feed-token <uid>`
...
- [ ] 4.1 Create `lib/Command/I18nExportStringsCommand.php` implementing `launchpad:i18n:export-strings`
  - Scans `lib/` (PHP) and `src/` (Vue/JS) for i18n markers
  - Writes output to `l10n/launchpad.pot`
  - Idempotent (overwrites existing file); exits 0
- [ ] 4.2 Create `lib/Command/I18nMigrateLanguageStructureCommand.php` implementing `launchpad:i18n:migrate-language-structure`
  ...
  - Prompts confirmation unless `--no-interaction`
- [ ] 4.3 Create `lib/Command/I18nCopyNavigationCommand.php` implementing `launchpad:i18n:copy-navigation --from=<lang> --to=<lang>`
...
- [ ] 5.1 Add `<commands>` block to `appinfo/info.xml` listing all 8 new command classes:
  - `OCA\LaunchPad\Command\DashboardListCommand`
  - `OCA\LaunchPad\Command\DashboardShowCommand`
  - `OCA\LaunchPad\Command\DashboardDeleteCommand`
  - `OCA\LaunchPad\Command\DashboardDebugShareCommand`
  - `OCA\LaunchPad\Command\UserRevokeFeedTokenCommand`
  - `OCA\LaunchPad\Command\I18nExportStringsCommand`
  - `OCA\LaunchPad\Command\I18nMigrateLanguageStructureCommand`
  - `OCA\LaunchPad\Command\I18nCopyNavigationCommand`
- [ ] 5.2 Verify `php occ list launchpad` lists all 8 commands after registration
- [ ] 5.3 Verify `php occ list launchpad:dashboard`, `php occ list launchpad:i18n`, `php occ list launchpad:user` filter correctly
