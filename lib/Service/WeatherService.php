...
class WeatherService
{
    ...
    public function resolveForPlacement(int $placementId, string $userId): array
    {
        ...
        $fresh = null;
        if ($location === '') {
            // No author-configured location — prefer the viewer's own
            // weather_status personal location, if available.
            $fresh = $this->fetchFromWeatherStatus(units: $units);
        }

        if ($fresh === null && $location !== '') {
            $fresh = $this->fetchFromProviderUrl(location: $location, units: $units, language: $language);
        }
    ...
    private function resolveUnits(string $userId, string $override): string
    {
        ...
        return in_array(needle: $territory, haystack: self::IMPERIAL_TERRITORIES, strict: true) === true ? 'imperial' : 'metric';
    }//end resolveUnits()
    ...
    private function normaliseMetNoForecast(array $forecast, array $locationInfo, string $units): ?array
    {
        ...
        return [
            'location'      => ($label !== '' ? $label : 'Current location'),
            'tempValue'     => $temp,
            'units'         => $units,
            'condition'     => $condition,
    ...
    private function normaliseProviderPayload(array $payload, string $location, string $units, string $language): ?array
    {
        ...
        return [
            ...
            'units'         => $units,
            'condition'     => $condition,
            'conditionText' => ($description !== '' ? ucfirst(string: $description) : $this->humaniseCondition(code: $condition)),
            'language'      => $language,
        ];
