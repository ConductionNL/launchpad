...
5. **Verify Completeness**
   ...
   **Task Completion**:
   ...
   - **For every task marked `[x]` in tasks.md** (whether already complete before this run, or just completed above), if plan.json exists and that task's `status` in plan.json is not `"done"`:
     - **Check off the verified criterion in the task issue body**:
       - **MCP (preferred):** `get_issue` → `{owner, repo, issue_number: <task_issue>}` → find all `- [ ]` criterion lines → change each to `- [x]` → `update_issue` → `{owner, repo, issue_number: <task_issue>, body: <updated_body>}`
       - **CLI (fallback):** `gh issue view <task_issue> --repo <repo> --json body --jq '.body'` → update all checkbox lines → `gh issue edit <task_issue> --repo <repo> --body "<updated_body>"`
     - **Check off this task in the tracking issue body**:
       - **MCP (preferred):** `get_issue` → `{owner, repo, issue_number: <tracking_issue>}` → find the task's line → change `- [ ]` to `- [x]` → `update_issue` → `{owner, repo, issue_number: <tracking_issue>, body: <updated_body>}`
       - **CLI (fallback):** `gh issue view <tracking_issue> --repo <repo> --json body --jq '.body'` → update task line → `gh issue edit <tracking_issue> --repo <repo> --body "<updated_body>"`
       - **IMPORTANT**: Batch all tracking issue updates into a single `update_issue` call — fetch the body once, apply all checkbox changes, then write it back once.
     - **Close task issue with comment** (if not already closed):
       - **MCP (preferred):** `update_issue` → `{owner, repo, issue_number: <task_issue>, state: "closed"}`, then `add_issue_comment` → `{owner, repo, issue_number: <task_issue>, body: "✓ Verified by /opsx:verify"}`
       - **CLI (fallback):** `gh issue close <task_issue> --repo <repo> --comment "✓ Verified by /opsx:verify"`
...
8. **Ask about API and browser testing**
   ...
   **If browser testing selected:**
   ...
   - **THEN**: `browser_snapshot` to verify expected outcome, `browser_take_screenshot` for evidence
...
11. **Final assessment and archive prompt**
   ...
   **If all tasks `[x]` AND only SUGGESTION issues or all clear:**
   ...
   - If plan.json exists, add a comment to the tracking issue:
