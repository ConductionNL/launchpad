...
- Add a new table `oc_launchpad_feed_cache` to persist feed fetch metadata (ETag, Last-Modified, last fetch time, error state) and cached items per feed URL.
- Register a Nextcloud `\OCP\BackgroundJob\TimedJob` named `FeedRefreshJob` that runs every 60 minutes (admin-tunable via `launchpad.feed_refresh_interval_seconds`, min 5 min, max 24h).
...
- Use appropriate User-Agent header to identify LaunchPad as the requestor.
...
- `lib/Migration/VersionXXXXDate2026...AddFeedCacheTable.php` — schema migration creating `oc_launchpad_feed_cache` table.
...
- `OCP\IConfig` — read proxy settings and app config (`launchpad.feed_refresh_interval_seconds`, `launchpad.news_widget_allowed_feed_hosts`).
...
- One schema migration to create `oc_launchpad_feed_cache` table.
- Zero data backfill required; job begins operating immediately after migration.
- Admin can immediately configure `launchpad.feed_refresh_interval_seconds` and `launchpad.news_widget_allowed_feed_hosts` via `IAppConfig`.

**Concurrency & Performance:**

- Per-feed locking using Nextcloud's `ILockingProvider` prevents concurrent fetches of the same URL.
- Batch processing: jobs with >500 feeds spread work across consecutive ticks using `cursor` state persisted in `oc_launchpad_feed_cache.processingCursor`.
