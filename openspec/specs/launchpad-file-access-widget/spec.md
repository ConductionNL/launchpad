# Spec: launchpad-file-access-widget
...
on a launchpad dashboard via a single widget
(`launchpad_file_access`). The widget is a **quick-access surface**
...
launchpad MUST NOT store file bytes. All file data flows live from
...
### REQ-FAW-003: The widget SHALL consume files via Nextcloud Files OCS / WebDAV — never via a launchpad-local file table
...
launchpad MUST NOT define a local file metadata table, MUST NOT cache
...
#### Scenario: No launchpad file table

- **GIVEN** the launchpad migration files after this widget ships
- **WHEN** inspected
...
- **WHEN** the widget renders
- **THEN** the row MUST display `t('launchpad', 'File not found')`
