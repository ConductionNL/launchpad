...
- Image sources: support external URLs with an allow-list (`launchpad.header_widget_allowed_image_domains`), and internal Nextcloud file IDs (via NC Files preview route) with file-read ACL checks.
...
- `header-widget` — A new LaunchPad dashboard widget capability providing full-width banners with images, overlays, text, and CTA buttons.
...
- `lib/Migration/VersionXXXXDate2026...AddHeaderWidgetSettings.php` — schema migration adding app config setting `launchpad.header_widget_allowed_image_domains` (JSON array; empty = all allowed).
