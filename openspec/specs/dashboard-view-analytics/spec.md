...
Unique-viewer dedup uses a 32-byte random salt regenerated each UTC midnight by `SaltRotationJob`, persisted to `IConfig` under `launchpad.analytics_dailysalt`, and **overwritten** (no history kept). The viewer hash is `sha256(userId || dailySalt)` and is stored only in the cache, keyed `launchpad_anlt_<dashboardUuid>_<hash>` with TTL set to the seconds remaining until the next UTC midnight so cache and salt lifetimes line up.

Two settings gate tracking:

| Setting | Scope | Default | Effect when off |
|---------|-------|---------|-----------------|
| `launchpad.analytics_enabled` | global | `true` | All view events are no-ops. |
| `launchpad.analytics_optout` | per-user pref | `false` | This user's view events are no-ops. |
