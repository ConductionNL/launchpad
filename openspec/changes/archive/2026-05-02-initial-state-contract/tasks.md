...
- [x] 2.2 Refactor `lib/Settings/LaunchPadAdmin::getForm` to construct `InitialStateBuilder(Page::ADMIN)`, call all setters, then `apply()` — note: codebase has `Settings/LaunchPadAdmin.php`, not `Settings/Admin/AdminSettings.php`
...
- [x] 3.5 Add a CI lint task (`npm run lint:initial-state` → `scripts/lint-initial-state.js`) that greps `loadState\(['"]launchpad['"]` outside `src/utils/loadInitialState.js` and fails the build if found
...
- [x] 4.8 CI lint pair (PHP grep + JS grep) — wired into `composer check:strict` (PHP) and runnable as `npm run lint:initial-state` (JS); add a step to `.github/workflows/code-quality.yml` if/when LaunchPad adopts a non-shared workflow (current setup uses the shared ConductionNL workflow which already runs `composer check:strict`)
