...
LaunchPad departs from this pattern in two deliberate ways. First, it stores global footer configuration in admin settings rather than the filesystem, because LaunchPad dashboards have two storage backends (database and GroupFolder) and a filesystem-only path is not a unified fit. Second, it adds a per-dashboard footer override (`dashboardFooterMode`) that does not exist in the source at all. Both departures are intentional LaunchPad additions.
...
- Provide a global footer toggle and content configuration via admin settings (`launchpad.footer_enabled`, `launchpad.footer_html`, `launchpad.footer_config`, `launchpad.footer_background_color`, `launchpad.footer_text_color`).
- Support two authoring modes — raw HTML and structured form-based config — selectable per instance.
- Allow per-dashboard footer override (`inherit` / `hidden` / `custom`) stored as columns on `oc_launchpad_dashboards`.
...
- Migrating the source's per-language `footer.json` filesystem layout to LaunchPad — admin settings is the single storage path.
...
**Decision**: Global footer config lives in five admin settings keys: `launchpad.footer_enabled`, `launchpad.footer_html`, `launchpad.footer_config`, `launchpad.footer_background_color`, `launchpad.footer_text_color`. Stored and retrieved via the existing `admin-settings` capability.

**Alternatives considered:**

- **Source's `{lang}/footer.json` filesystem-per-language pattern**: rejected because LaunchPad dashboards can be stored in either the database or a GroupFolder, so a filesystem path is not a universal anchor. Admin settings provide a single authoritative store regardless of the active storage backend.
...
**Rationale**: Admin settings is already the established pattern for LaunchPad instance-wide configuration. Reusing it keeps the bootstrap path simple, avoids filesystem permission complexity, and makes the footer values available via `IAppConfig` without mounting any GroupFolder.

### D2: Per-dashboard override — LaunchPad addition

**Decision**: Each dashboard MAY carry a footer override via two new columns on `oc_launchpad_dashboards`: `dashboardFooterMode VARCHAR(16)` (values: `inherit`, `hidden`, `custom`; default `inherit`) and `dashboardFooterHtml MEDIUMTEXT NULL` (populated only when mode is `custom`). When mode is `inherit`, the dashboard renders the global footer. When `hidden`, no footer renders. When `custom`, the dashboard's own HTML renders (sanitised identically to global HTML).

**Source evidence**: None — this is entirely a LaunchPad addition. The source has no per-page footer concept.
...
**Source evidence**: No explicit print handling found in the source footer implementation — this is a LaunchPad addition.
...
**Decision**: Footer background and text colours follow NC theme CSS variables (`--color-main-background`, `--color-main-text`) by default. If `launchpad.footer_background_color` or `launchpad.footer_text_color` are set (valid CSS hex strings), those override the theme variables inline on the root footer element.
...
**Decision**: `launchpad.footer_enabled` defaults to `false`. A fresh LaunchPad installation renders no footer until an admin explicitly enables it via the admin settings UI or API.

**Rationale**: An empty or placeholder footer on day one is visually awkward and may confuse end users. Defaulting to off means the footer only appears once the admin has configured meaningful content, matching the principle of safe defaults across LaunchPad admin settings.
...
- **REQ-FTR-006** (per-dashboard override): add a NOTE marking this requirement as a LaunchPad addition with no source counterpart, citing D2. Confirm storage as `dashboardFooterMode VARCHAR(16)` + `dashboardFooterHtml MEDIUMTEXT NULL` on `oc_launchpad_dashboards`.
