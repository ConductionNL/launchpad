...
- [x] **T03**: Create `SettingsTableBuilder::create()` to define the `launchpad_admin_settings` table schema (BIGINT PK, VARCHAR(255) unique key, TEXT nullable value, DATETIME updated_at) — `lib/Migration/SettingsTableBuilder.php`
...
- [x] **T13**: Implement `LaunchPadAdminSection` (`IIconSection`) registering the `launchpad` admin navigation section with icon, name, and priority — `lib/Settings/LaunchPadAdminSection.php`

- [x] **T14**: Implement `LaunchPadAdmin` (`ISettings`) that enqueues the `launchpad-admin` script bundle and returns the `settings/admin` template response — `lib/Settings/LaunchPadAdmin.php`

- [x] **T15**: Register `LaunchPadAdmin` and `LaunchPadAdminSection` in `appinfo/info.xml` under `<settings>` — `appinfo/info.xml`
...
- [x] **T19**: Create `src/admin.js` entry point bootstrapping Vue 2 + Pinia app mounting `AdminSettings.vue` on `#launchpad-admin-settings` — `src/admin.js`
