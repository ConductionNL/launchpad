...
# Header Widget — Delta from change `header-widget`
...
The system MUST store per-placement widget configuration in the `oc_mydash_widget_placements.content` JSON field (persisted shape: `{type: 'header', content: {...}}`), allowing users to specify content, styling, image source, and optional CTA.
...
- AND the default value MUST be `var(--color-primary)`
...
#### Scenario: Config for overlay color

- GIVEN a user is configuring a header widget placement
- WHEN they set the overlay color
- THEN the configuration MUST store an `overlayColor: string|null` field
- AND the default behavior (when null) MUST fall back to `backgroundColor` then `#000000`
...
- THEN the `content` MUST deserialize cleanly to a JavaScript object
...
- GIVEN a placement with overlayMode='tint', overlayOpacity=0.4, and overlayColor='#000000'
- WHEN the widget renders
...
- GIVEN a placement with overlayMode='gradient-bottom' and overlayColor='#000000'
- WHEN the widget renders
- THEN a linear gradient overlay MUST be applied: top=transparent, bottom=overlayColor
