...
LaunchPad currently has no branded footer mechanism. Organizations need to display copyright, legal links, contact information, and custom branding (logos, organization name, address) below the dashboard content. Without a configurable footer, instances lack a professional closure and legal disclaimers, forcing admins to inject HTML at the reverse-proxy level or rely on Nextcloud theme customizations that don't directly integrate with the dashboard surface.

## What Changes

- Add two global admin settings: `launchpad.footer_enabled` (boolean, default `false`) and `launchpad.footer_html` (sanitised HTML string, max 8KB).
- Add structured configuration alternative: `launchpad.footer_config` (JSON object) with schema `{logoUrl?, organisation?, address?, links: [{label, url}]?, legal?, copyrightYear?, layoutMode: 'columns'|'inline'}` — use when admin prefers form-based input over raw HTML.
- Add admin UI in LaunchPad settings: tabbed "HTML Mode" vs "Structured Mode" with live preview.
...
- Theme awareness: footer text + background colours follow NC theme variables; admin MAY override with `launchpad.footer_background_color` and `launchpad.footer_text_color` settings.
