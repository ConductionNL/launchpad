# Context Brief: Compliance and audit dashboard panel

**App:** MyDash — Dashboard system
**Spec:** compliance-audit-panel
**Platform:** Nextcloud + OpenRegister

## Features (2 total, sorted by market demand)

### Receive a dashboard alert when a compliance deadline is imminent
**demand: 45** (15 tender mentions) | Category: governance | **Coverage: mydash**

CAPABILITY: As a compliance officer, I want to receive a dashboard alert when a compliance deadline is imminent, so that I can take timely action and avoid regulatory violations. ACCEPTANCE CRITERIA:
1. GIVEN a compliance deadline is within the configured alert threshold, WHEN the officer opens the mydash dashboard, THEN a visible alert is displayed identifying the deadline and its due date.
2. GIVEN an imminent compliance deadline alert is displayed, WHEN the officer has not acknowledged it, THEN the alert persists on the dashboard across sessions until dismissed or the deadline passes.
3. GIVEN a compliance deadline has passed, WHEN the officer views the dashboard, THEN the imminent alert is replaced or updated to reflect the overdue status.

### Filter governance dashboard by portfolio
**demand: 42** (14 tender mentions) | Category: governance | **Coverage: mydash**

CAPABILITY: As a portfolio manager, I want to filter the governance dashboard by portfolio, so that I can focus on relevant governance data for my area. ACCEPTANCE CRITERIA:
1. GIVEN the governance dashboard is open, WHEN the portfolio manager selects a specific portfolio from the filter, THEN only governance items belonging to that portfolio are displayed.
2. GIVEN a portfolio filter is active, WHEN the portfolio manager clears the filter, THEN all governance items across all portfolios are shown.
3. GIVEN multiple portfolios exist, WHEN the portfolio manager applies a portfolio filter, THEN the dashboard summary metrics update to reflect only the selected portfolio's data.
