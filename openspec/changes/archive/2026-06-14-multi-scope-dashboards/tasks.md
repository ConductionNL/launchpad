...
- [x] Task 13: File the follow-up `admin-group-management` change for the admin-facing group-shared CRUD UI and note the deferral in the changelog — `openspec/changes/admin-group-management/` filed with `proposal.md` + `tasks.md` (11 tasks) + `specs/dashboards/spec.md` (one new REQ-DASH-015 with four scenarios) + `hydra.json` (filed-status sentinel). CHANGELOG.md gets a new `### Deferred` section under `## Unreleased` that points to the filed change so the next planner can pick it up without searching
