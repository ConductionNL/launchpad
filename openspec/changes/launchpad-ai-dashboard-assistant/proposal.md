...
- `launchpad-cases-summary` — display open case count, status breakdown, and AI summary
- `launchpad-consultation-summary` — display consultation response metrics and key feedback summary

## Notes

- Summaries are generated on-demand (not pre-computed) and cached briefly to balance freshness vs. API cost.
- Both widgets integrate with LaunchPad's existing dashboard edit mode and GridStack layout engine.
