...
# Add launchpad discovered widgets + spend-analytics widget

## Why

Specter's intelligence pass for launchpad surfaced 14 spec drafts that have no
match in the existing `openspec/specs/` library. launchpad today ships widget
...
compliance), but a Nextcloud user landing on launchpad sees only the
generic widget bundle. This change adds the missing capabilities as
**widget specs** — launchpad's first-class extensibility surface — so the
...
intelligence brief. They are consolidated into one launchpad widget spec
(`launchpad-spend-analytics-widget`) that frames the capability as a
launchpad widget consuming runtime GraphQL from financeq + procest +
docudesk — per `feedback_launchpad-no-or-dependency.md` launchpad MUST NOT
...
`mobile-remote-access`) each describe a distinct launchpad widget surface
and become their own specs.

## What Changes

Seven net-new capability specs, all under the existing launchpad widget
...
- **launchpad-spend-analytics-widget** — spend-analytics dashboard widget
...
- **launchpad-ai-dashboard-assistant** — embedded AI assistant widget
  ...
  openconnector) per ADR-022; launchpad does NOT host its own LLM client.
- **launchpad-compliance-audit-panel** — compliance posture widget
...
- **launchpad-file-access-widget** — file browser / quick-access widget
  ...
  file storage in launchpad.
- **launchpad-enterprise-security-access** — read-only security/access
  panel surfacing role assignments, SSO status, MFA enforcement.
  Consumes OR's RBAC abstraction per ADR-022.
- **launchpad-meeting-calendar-actions** — meeting + calendar actions
  widget surfacing upcoming meetings from Nextcloud Calendar (DAV)
  + decidesk meetings/agendas (runtime GraphQL).
- **launchpad-mobile-remote-access** — responsive-widget + PWA capability
...
- `openspec/specs/launchpad-*/spec.md` — 7 new capability specs.
- `openspec/changes/add-launchpad-discovered-and-spend-analytics/` — this
...
- (deferred to per-spec implementation chains) every widget added by
  ...
  apps. No new endpoints on launchpad. Runtime GraphQL endpoints
...
- **No new install-time dependencies.** Per
  `feedback_launchpad-no-or-dependency.md` + ADR-024 §10, launchpad's
...
- Implementation defers the "which GraphQL schemas exist on each
  ...
  missing GraphQL field. launchpad itself stays unblocked.
...
- AI-model selection, prompt engineering, RAG plumbing for the
  AI assistant — `launchpad-ai-dashboard-assistant` describes the
  widget surface and the openconnector hand-off, not the model
  side.
- New launchpad routes or pages — every capability lands as a widget on
  the existing dashboard surface; no new manifest `pages[]` entries.
- PWA service-worker / offline-storage implementation —
  `launchpad-mobile-remote-access` describes the manifest capability
  declarations only.
